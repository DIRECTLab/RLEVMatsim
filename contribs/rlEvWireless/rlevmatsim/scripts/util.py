...
def save_server_output(save_dir, response, filetype):
    """
    ...
    """
    zip_filename = Path(save_dir, f"{filetype}.zip")
    extract_folder = Path(save_dir, filetype)
...
def send_reward_request(save_dir, dataset, time_string):
    ...
    if filetype == "initialoutput":
        save_server_output(save_dir, response, filetype)
